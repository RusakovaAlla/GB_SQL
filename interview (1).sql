...
/* Соединяем таблицы users и posts по полям id и user_id соответственно.
Таблица posts содержит больше значений user_id, чем таблица users содержит id. Т.е. часть данных по пользователям утеряна или ошибочно внесена информация?
Сопоставление двух таблиц в полном объеме невозможно, удалим часть записей
*/
DELETE 
FROM posts 
