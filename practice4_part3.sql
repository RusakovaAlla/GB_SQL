DROP DATABASE IF EXISTS practice4_part3;
CREATE DATABASE practice4_part3;
USE practice4_part3;
...
SELECT count(user_id) AS likes_under_12_yo
FROM likes
WHERE user_id IN (SELECT user_id
				  FROM profiles
                  WHERE birthday>DATE_SUB(NOW(), INTERVAL 12 Year));
                  
-- Определить кто больше поставил лайков (всего): мужчины или женщины  
SELECT profiles.gender, count(likes.id) AS max_likes
FROM likes
LEFT JOIN profiles
...
WHERE user_id NOT IN (SELECT DISTINCT(from_user_id) FROM messages);   
 ...
 -- Пусть задан некоторый пользователь. Из всех друзей этого пользователя найдите человека, который больше всех написал ему сообщений. Пусть user_id=1
 SELECT from_user_id AS sender, count(body)
 FROM messages
 WHERE to_user_id = 1 AND 
 from_user_id IN (SELECT initiator_user_id 
